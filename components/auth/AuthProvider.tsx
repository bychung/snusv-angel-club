...
export default function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // pathname 변경을 감지해서 리다이렉트 완료 시 로딩 해제
  useEffect(() => {
    if (isRedirectingRef.current && !pathname.startsWith('/survey')) {
      console.log('[AuthProvider] Redirect completed, pathname changed to:', pathname);
    ...
    supabase.auth.getUser().then(({ data: { user }, error }) => {
      ...
      handleAuthenticatedUserRouting(user, true)
        .then(routingResult => {
          ...
          if (!routingResult.startsWith('redirecting-')) {
            setLoading(false);
          } else {
            console.log(
              '[AuthProvider] Redirect in progress, loading will be cleared on pathname change'
            );
          }
        })
        .catch(error => {
        ...
        if (event === 'SIGNED_IN') {
          ...
          handleAuthenticatedUserRouting(session.user, false)
            .then(routingResult => {
              ...
              if (routingResult.startsWith('redirecting-')) {
                console.log(
                  '[AuthProvider] SIGNED_IN redirect in progress, loading will be cleared on pathname change'
                );
                return routingResult; // 상태 전달
              }
