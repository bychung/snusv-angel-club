  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isLoggedInUser && user) {
        // 로그인된 사용자인 경우: 기존 프로필 조회 후 업데이트 (브랜드별)
      ...
      if (profile && oldProfileData) {
        ...
        if (importantFieldChanges.length > 0) {
          ...
          results.forEach((result, index) => {
            if (result.error) {
              console.error(
                `프로필 변경 이력 저장 실패 (${importantFieldChanges[index].field_name}):`,
                result.error
              );
      ...
      if (upsertedFundMember) {
        if (isNewApplication) {
          ...
          if (changeHistoryError) {
            console.error('출자 변경 이력 저장 실패:', changeHistoryError);
          }
        }
      }

      // 성공 시 profileId 저장 (회원가입시 매칭용)
      store.setProfileId(activeFundId, profileId);
