...
interface TemplateDiffModalProps {
  isOpen: boolean;
  onClose: () => void;
  templates: DocumentTemplate[]; // 버전 히스토리 (최신순)
...
export default function TemplateDiffModal({
  ...
  }, [isOpen, defaultFromId, defaultToId, templates]);
  ...
  const fetchDiff = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/admin/templates/diff?from=${fromId}&to=${toId}`
      );

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || '템플릿 비교 실패');
      }
  ...
  const getVersionLabel = (template: DocumentTemplate) => {
